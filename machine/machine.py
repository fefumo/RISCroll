...
def should_jump(cpu: CPU, condition):
    match condition:
        ...
        case "GT": return cpu.flags["N"] == 0 and cpu.flags["Z"] == 0
        case "LE": return cpu.flags["N"] == 1 or cpu.flags["Z"] == 1
        case _: return False
...
def extract_operands(cpu: CPU, mi: MicroInstruction):
    ...
    if mi.latch_alu in {"add", "and", "or"} and opcode == 0x13:
        ...
        imm_j = ((ir >> 31) << 20) | (((ir >> 21) & 0x3FF) << 1) | (((ir >> 20) & 0x1) << 11) | (((ir >> 12) & 0xFF) << 12)
        if imm_j & (1 << 20):
            imm_j |= ~((1 << 21) - 1)
        return cpu.pc, imm_j
    return 0, 0  # fallback
